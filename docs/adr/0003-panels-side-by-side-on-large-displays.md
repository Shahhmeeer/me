...
side by side on the Strip and the visitor slides between them: a wheel roll,
the arrow keys, a Nav link or a hash in the address each move it one Panel.
Below that, and on any display held upright, the Panels stack top to bottom
and the site scrolls the way every phone site does.

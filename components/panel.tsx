...
type PanelProps = {
   ...
   */
  isHeadline?: boolean;
  /** A short line above the heading. Home's greeting; nothing else has one. */
  above?: string;
  children: ReactNode;
};
 ...
 *
 * Panels stack top to bottom on every display today. The strip that sits
 * them side by side on a large display comes next, and changes nothing here.
 */
  ...
  return (
    ...
    >
      <div className="flex flex-col gap-2">
        {above !== undefined ? (
          <p className="text-lead text-muted">{above}</p>
        ) : null}
        {isHeadline ? (
          <h1
            id={headingId}

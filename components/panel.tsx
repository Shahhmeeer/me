 ...
 * heading is an h2, the Panel's Nav label; Home is headed by the Headline,
 * the page's one h1, and is laid out its own way in
 ...
 * Panel is a row: the left column, the heading, is sticky and stays put
 * while the content to its right slides past. The content is a row of blocks
 * that grows the Panel sideways to fit; what is taller than the screen is
 * clipped, never scrolled, because the Strip has one direction. The column
 * is painted on the page colour so what slides under it is hidden rather
 * than read through it. Which screen a Panel is on is decided once, by the
 * `large` variant in `app/globals.css`, and nowhere here; what every Panel
 * on the Strip shares, the snap point and the screen, is the `.panel` rule
 * there.
 */
export function Panel({ id, heading, children }: PanelProps) {
  ...
  return (
    <section
      ...
      className="panel mx-auto flex min-h-svh w-full max-w-3xl flex-col justify-center gap-gutter px-gutter pt-nav pb-section large:w-max large:flex-row large:items-start large:justify-start large:pl-0 large:pb-gutter"
    >

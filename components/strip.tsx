 ...
 *
 * This is the second client component on the site, after the reveal, and it
 * does three small things the browser cannot do on its own. Everything else
 ...
 * it as it was.
 *
 * It turns the wheel sideways. On a large display the strip scrolls
 * sideways and the document does not scroll at all, so a wheel rolled the
 * way every wheel is rolled would do nothing. A vertical roll moves the
 * strip one screen the way it points; a sideways swipe is left to the
 * browser, which already scrolls the strip with it. The move is a screen and
 * not the wheel's own distance because the strip snaps to a Panel: a smaller
 * scroll is snapped straight back to where it began.
 *
 ...
 * should still move. The strip is focusable too, so a Tab can land on it and
 * a screen reader can name the thing the keys move.
 *
...
export function Strip({ children }: StripProps) {
  ...
  useEffect(() => {
    ...
    const onWheel = (event: WheelEvent) => {
      ...
      moveScreens(node, event.deltaY > 0 ? 1 : -1);
    };
    ...
    const onKeyDown = (event: KeyboardEvent) => {
      ...
      if (event.key === "ArrowRight" || event.key === "ArrowLeft") {
        ...
        moveScreens(node, event.key === "ArrowRight" ? 1 : -1);
      }
    };
    ...
    // A browser with no observer keeps the wheel and the keys, and the Nav
    // keeps Home lit.
    const observer =
